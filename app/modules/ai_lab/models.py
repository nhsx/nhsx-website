...
class AiLabResourceIndexPage(RoutablePageMixin, BasePage):
    ...
    def all_resources(self, request):
        ...
        return render(
            request,
            "ai_lab/ai_lab_resource_index_page.html",
            {
    ...
    def filter_by_use_case(self, request, slug):
        ...
        if children:
            return render(
                request,
                "ai_lab/ai_lab_resource_index_page.html",
                {"use_case": children[0].use_case, "page": self, "children": children,},
            )
        else:
            raise Http404
    ...
    def _get_resource_ids_for_use_case(self, slug):
        ...
        for resource_ids_for_use_case in [
            self._get_ids_for_class(klass, slug) for klass in self.subpage_types
        ]:
